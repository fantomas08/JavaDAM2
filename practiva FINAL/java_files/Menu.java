...
public class Menu {
		...
		if (contacts.isEmpty()) {
			...
			if (contacts.size() > 1) {
				...
				if (Integer.parseInt(input) > contacts.size() || Integer.parseInt(input) < 0) {
					...
					contact = contacts.get(Integer.parseInt(input));
				}
			} else {
	...
	public static void main(String[] args) {
		...
		emails.add("email1");
		emails.add("email2");
		schedule.getList().add(new Contact("Yevgen", "Chaynykov", "", emails, "", "", mobiles));
		schedule.getList().add(new Contact("Anton", "Sachkov", "", new ArrayList<String>(), "", "", mobiles));
		
		//System.out.println(schedule.getList().toString());
		
		schedule.exportContacts("Text", "txt");
		//schedule.exportContacts("schedule", "bin");
		//schedule.exportContacts("XML", "xml");
		
		schedule.importContacts(new File("Text.txt"));
		//schedule.importContacts(new File("schedule.bin"));
		//schedule.importContacts(new File("XML.xml"));
